...
func colorCubeFilterFromBase64(_ string: String) -> NSData? {
	guard
		...
		let imageSource = CGImageSourceCreateWithData(baseData as CFData, nil),
		let lut3DImage = CGImageSourceCreateImageAtIndex(imageSource, 0, nil)
	else { return nil }
	return colorCubeFilterFromLUT(lut3DImage)
}

func colorCubeFilterFromLUT(_ image: CGImage) -> NSData? {
	let size = Float(64)

	let lutImage = image
	let lutWidth = Float(lutImage.width)
	let lutHeight = Float(lutImage.height)
	let rowCount = lutHeight / size
	let columnCount = lutWidth / size

	if (lutWidth.truncatingRemainder(dividingBy: size) != 0) || (lutHeight.truncatingRemainder(dividingBy: size) != 0) ||
...
func getBytesFromImage(image: CGImage?) -> [UInt8]? {
...
private func cubeDataForLut64(_ lutImage: CIImage) -> Data? {
	...
	guard (imageWidth % cubeDimension == 0) || (imageHeight % cubeDimension == 0) ||
		(rowCount * columnCount == cubeDimension) else {
		print("Invalid LUT")
		return nil
	}
...
private func createRGBABitmapFromImage(_ image: CIImage) -> UnsafeMutablePointer<Float> {
	let bitsPerPixel = 32
	let bitsPerComponent = 8
	...
	vDSP_vfltu8(
		UnsafePointer<UInt8>(bitmapData!.assumingMemoryBound(to: UInt8.self)),
		1,
		UnsafeMutablePointer<Float>(convertedBitmap!.assumingMemoryBound(to: Float.self)),
		1,
