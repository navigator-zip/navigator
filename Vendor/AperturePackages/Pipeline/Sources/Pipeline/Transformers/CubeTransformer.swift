...
public final class CubeTransformer {
	...
	public static func loadInitialPipelineData() async throws {
		try loadInitialPipelineDataSync()
	}

	public static func loadInitialPipelineDataSync() throws {
		if CubeTransformer.chromaticFoliaData == nil { CubeTransformer.chromaticFoliaData = colorCubeFilterFromBase64(folia) }
		if CubeTransformer
			.chromaticSupergoldData == nil { CubeTransformer.chromaticSupergoldData = colorCubeFilterFromBase64(supergold) }
		if CubeTransformer
			.chromaticTonachromeData == nil { CubeTransformer.chromaticTonachromeData = colorCubeFilterFromBase64(vertichrome) }
		if CubeTransformer
			.monochromaticData == nil { CubeTransformer.monochromaticData = colorCubeFilterFromBase64(mononoke) }
		if CubeTransformer
			.monochromaticFrontData ==
			nil { CubeTransformer.monochromaticFrontData = colorCubeFilterFromBase64(mononokeFrontTwo) }
